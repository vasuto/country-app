...
angular.module('countryDevApp', ['countryApp', 'ngMockE2E'])
.run(['$httpBackend', '$http', function($httpBackend, $http) {
    var selectedCountries = [];

    $httpBackend.whenGET(/country-view\/.*/).passThrough();    // Requests for templates are handled by the real server
    $httpBackend.whenGET(/components\/.*/).passThrough();   // Requests for templates are handled by the real server

    /**
     * Returns the list of countries filtered out by the search string.
     */
    $httpBackend.whenGET(/country?.*/).respond(function(method, url, data, headers, params) {
        var list = _.filter(countries, function(country) {
            if(params.search) {
                return country.name.toLowerCase().startsWith(params.search.toLowerCase());
            }
        });
        if (list.length === 0) {
            return [404, {}, {}];;
        } else {
            return [200, JSON.stringify(list), {}];;
        }
    });

    /**
     * Adds countries to the selected countries array.
     */
    $httpBackend.whenPOST('/selectedCountries').respond(function(method, url, data) {
        var countries = angular.fromJson(data);
        selectedCountries = selectedCountries.concat(countries);
